...
import com.cropdeal.delivery.client.OrderServiceClient;
import com.cropdeal.delivery.client.PaymentServiceClient;
import com.cropdeal.delivery.config.NotificationRabbitConfig;
...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    private void publishDeliveryAccepted(Delivery delivery) {
        try {
            OrderLookupResponse order = orderServiceClient.getOrderById(delivery.getOrderId());
            DeliveryAcceptedEvent event = new DeliveryAcceptedEvent(
                    delivery.getId(),
                    delivery.getOrderId(),
                    order.getFarmerId(),
                    order.getDealerId(),
                    delivery.getDeliveryPartnerId(),
                    delivery.getAcceptedBy(),
            ...
            rabbitTemplate.convertAndSend(
                    ...
                    NotificationRabbitConfig.DELIVERY_ACCEPTED_ROUTING_KEY,
                    event
            );
        } catch (Exception e) {
            log.warn("Failed to publish delivery accepted notification for delivery {}: {}",
    ...
    private void publishDeliveryCompleted(Delivery delivery) {
        try {
            ...
            rabbitTemplate.convertAndSend(
                    NotificationRabbitConfig.NOTIFICATION_EXCHANGE,
                    NotificationRabbitConfig.DELIVERY_COMPLETED_ROUTING_KEY,
                    event
            );
        } catch (Exception e) {
            log.warn("Failed to publish delivery completed notification for delivery {}: {}",
                    delivery.getId(), e.getMessage());
        }
    }
